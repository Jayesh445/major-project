...
interface OptimizationState {
  ...
  analysis?: {
    overstockedWarehouses?: string[];
    understockedWarehouses?: string[];
    imbalancedProducts?: string[];
    insights?: string[];
  };
  recommendations: {
    productSku: string;
...
async function executeOptimizationWorkflow(): Promise<OptimizationState> {
  ...
  try {
    ...
    const warehousesResult = await fetchWarehousesTool.invoke({});
    ...
    const inventoryResult = await fetchInventoryDataTool.invoke({});
    ...
    const analysisPrompt = ANALYSIS_PROMPT(state.warehouses, state.inventoryData);

    const analysisResult = await model.invoke([
      { role: 'user', content: analysisPrompt }
    ]);

    state.analysis = JSON.parse(analysisResult.content as string);
    ...
    const optimizationPrompt = OPTIMIZATION_PROMPT(
      state.warehouses,
      state.inventoryData,
      state.analysis
    );
    ...
    const optimization = JSON.parse(optimizationResult.content as string);
    ...
    for (const rec of state.recommendations) {
      ...
      const sourceInventoryData = state.inventoryData.find(
        inv => inv.warehouseId === fromWarehouse.id
      );
