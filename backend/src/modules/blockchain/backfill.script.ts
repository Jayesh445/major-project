...
import { pollConfirmation, updateLogStatus, getPendingLogs } from './service';
...
async function backfillPendingTransactions() {
  ...
  try {
    const pendingLogs = await getPendingLogs(1000); // Get up to 1000 pending logs
    ...
    for (const log of pendingLogs) {
      try {
        ...
        if (result.status === 'confirmed' && result.blockNumber) {
          ...
          console.log(
            `  ✓ Confirmed on block #${result.blockNumber}`
          );
          confirmed++;
...
export { backfillPendingTransactions };
